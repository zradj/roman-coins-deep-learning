...
3. **Stratified Split** - a custom stratified 80/20 split was generated with `random_seed=42`, yielding 14,075 training images and 3,472 test images, each with equal per-class representation.
4. **Model Training** - the KAN-Mixer was trained from scratch with Adam (lr=1e-3), cross-entropy loss, batch size 128, 12 epochs on a GPU.
5. **Evaluation** - checkpoints were saved after every epoch. The best checkpoint (epoch 12) was used for final comprehensive evaluation, including macro/micro/weighted F1, precision, recall, Top-5 and Top-10 accuracy, per-class F1.
...
No additional `pip install` steps are needed in a standard Colab Pro environment, since all of these are available by default. The custom KAN-Mixer model code lives in the project's GitHub repository and is pulled in at runtime (see setup instructions below).
...
This pulls in the `KAN_Mixer/` directory containing `model.py` and `config.py`, which define the architecture and configuration utilities.
...
- **Training cell** - trains KAN-Mixer for `num_epochs` epochs, saving a `.pt` checkpoint after each one to `weights_coins/`.
...
1. Run `evaluate_model_comprehensive()` on the test loader.
...
The primary path to improvement in Phase 2 is clear: introduce data augmentation to close the train-test gap, increase input resolution to 128×128 or 256×256 to recover fine inscription detail, and investigate the Class 16 / Class 60 bidirectional confusion, which is likely either a dataset labelling issue or an inherent visual ambiguity that requires domain-expert input to resolve. Additionally, we would add more data by merging the existing dataset with RRC60 dataset which would let our model achieve greater performance.